...
**Основной способ проверки — [локальный запуск через Docker](#установка-и-запуск).** Он запускает полный проект с PostgreSQL и не зависит от демонстрационных ссылок. [Тестовые аккаунты для жюри](#тестовые-аккаунты-для-жюри) создаются автоматически.

**Временный онлайн-предпросмотр:** [открыть Sana Quest](https://occurring-exciting-done-leader.trycloudflare.com). Он доступен только пока работают компьютер команды, Docker и Cloudflare Quick Tunnel. При выключении компьютера ссылка перестанет работать; постоянный сервер, независимый от ноутбука, пока не настроен. Если предпросмотр недоступен, используйте Docker-запуск ниже.

В предпросмотре используются отдельная база и те же тестовые аккаунты. Доступны регистрация, задачи, локальный помощник, отклики, файлы, отзывы и видео без голоса. Внешние AI-сервисы в нём отключены; их проверка требует локального запуска с API-ключами. Подробности и готовый пример видео — в [инструкции к демо](docs/public-demo.md).

Для постоянного размещения нужен внешний сервер с Docker, постоянным хранилищем и HTTPS. Исходный код приложения для такого переноса менять не требуется. [GitHub Pages](https://docs.github.com/en/pages/getting-started-with-github-pages/what-is-github-pages) размещает статические сайты и не запускает используемые здесь Go API, PostgreSQL и видеорендер.
...
Основной репозиторий проекта — `BAITC-Hacks/hack-654d40a5-lexus`, выданный организатором. Отправка кода в GitHub и действие «Сдать решение» на платформе хакатона выполняются отдельно. Контрольный срок финальной отправки команды — **17:50**. Для воспроизводимой проверки используйте локальный запуск на **http://localhost:8080**; актуальный статус внешнего размещения указан в начале README.
