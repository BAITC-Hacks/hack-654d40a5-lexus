...
[Запуск](#установка-и-запуск) · [Тестовые аккаунты](#тестовые-аккаунты-для-жюри) · [Сценарий проверки](#как-проверить-основной-сценарий) · [Критерии оценки](#соответствие-критериям-хакатона) · [Ошибки](#ошибки-и-восстановление)

## Публичное демо

**Открыть платформу: [https://occurring-exciting-done-leader.trycloudflare.com](https://occurring-exciting-done-leader.trycloudflare.com)**

Для входа используйте [тестовые аккаунты ниже](#тестовые-аккаунты-для-жюри). Это отдельный демонстрационный экземпляр с собственной базой: изменения в нём не затрагивают основной локальный проект.

Демо опубликовано через **Cloudflare Quick Tunnel** и доступно, пока запущены компьютер команды, Docker и контейнер туннеля. Это временный адрес, а не постоянный облачный сервер; после перезапуска туннеля адрес может измениться. Если ссылка недоступна, проект можно самостоятельно запустить по инструкции ниже. [Условия работы Quick Tunnel](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/trycloudflare/).

В публичном экземпляре работают регистрация, тестовые роли, локальный помощник, задачи, отклики, вложения, оценки и видео без озвучивания. Внешние AI-сервисы и распознавание аудио в нём отключены: личные ключи команды в общий тестовый экземпляр не передаются. Реальные интеграции проверены в локальной версии с настроенным `.env`; результаты описаны в [отчёте проверки](docs/verification.md).

Готовый пример: [видео по задаче «Карта доступного города»](https://occurring-exciting-done-leader.trycloudflare.com/media/d4f57d2b1127c534c8aa07ce013d1bde/final.mp4), 30 секунд, без озвучивания.

Конфигурация демо: [compose.demo.yaml](compose.demo.yaml). Запуск и обслуживание: [docs/public-demo.md](docs/public-demo.md).
...
Основной репозиторий проекта — `BAITC-Hacks/hack-654d40a5-lexus`, выданный организатором. Отправка кода в GitHub и действие «Сдать решение» на платформе хакатона выполняются отдельно. Контрольный срок финальной отправки команды — **17:50**. Для проверки доступны публичное демо по ссылке в начале README и локальный запуск на **http://localhost:8080**.
